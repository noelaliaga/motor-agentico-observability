/**
 * The web layer can read but never write.
 *
 * Runs with plain Node (`npm test`): `node --experimental-strip-types --test`.
 * db.ts starts with `import "server-only"`, which throws outside a React
 * Server Components build; the test registers an empty module in its place
 * before importing db.ts. Everything else is the real code.
 */
import { test, before, after } from "node:test";
import assert from "node:assert/strict";
import { mkdtempSync, rmSync } from "node:fs";
import { join } from "node:path";
import { createRequire } from "node:module";

const require = createRequire(import.meta.url);
const { DatabaseSync } = require("node:sqlite") as typeof import("node:sqlite");

// Swap "server-only" for an empty module (synchronous resolve hook, Node >= 22.15).
type Resolver = (s: string, c: unknown) => { url: string; shortCircuit?: boolean };
const nodeModule = require("node:module") as {
  registerHooks: (h: { resolve: (s: string, c: unknown, next: Resolver) => ReturnType<Resolver> }) => void;
};
nodeModule.registerHooks({
  resolve(spec, ctx, next) {
    if (spec === "server-only") return { url: "data:text/javascript,export {}", shortCircuit: true };
    return next(spec, ctx);
  },
});

// The temp dir lives inside the repo (ignored by git), not in the system /tmp.
const dir = mkdtempSync(join(import.meta.dirname, "..", ".tmp-test-"));
const path = join(dir, "motor.sqlite");

let lib: typeof import("../src/lib/db.ts");

before(async () => {
  const db = new DatabaseSync(path);
  db.exec("CREATE TABLE usage (id INTEGER PRIMARY KEY, model TEXT); INSERT INTO usage (model) VALUES ('a'), ('b');");
  db.close();
  process.env.MOTOR_DB = path;
  lib = await import("../src/lib/db.ts");
});

after(() => rmSync(dir, { recursive: true, force: true }));

function count(): number {
  const db = new DatabaseSync(path, { readOnly: true });
  const n = (db.prepare("SELECT COUNT(*) n FROM usage").get() as { n: number }).n;
  db.close();
  return n;
}

test("reads rows as plain objects (no null prototype)", () => {
  const rs = lib.rows<{ model: string }>("SELECT model FROM usage ORDER BY id");
  assert.deepEqual(rs.map((r) => r.model), ["a", "b"]);
  assert.equal(Object.getPrototypeOf(rs[0]), Object.prototype);
  assert.equal(lib.exists(), true);
});

test("an INSERT through the web layer is rejected by SQLite", () => {
  const warnings: string[] = [];
  const original = console.warn;
  console.warn = (...a: unknown[]) => { warnings.push(a.join(" ")); };
  try {
    const r = lib.rows("INSERT INTO usage (model) VALUES ('c') RETURNING id");
    assert.deepEqual(r, []);
  } finally {
    console.warn = original;
  }
  assert.equal(count(), 2, "the row must not have been written");
  assert.match(warnings.join("\n"), /readonly|read-only|read only/i);
});

test("DELETE, DROP and CREATE TEMP are rejected too, for being read-only", () => {
  for (const sql of ["DELETE FROM usage", "DROP TABLE usage", "CREATE TEMP TABLE probe (x)"]) {
    const warnings: string[] = [];
    const original = console.warn;
    console.warn = (...a: unknown[]) => { warnings.push(a.join(" ")); };
    try {
      assert.deepEqual(lib.rows(sql), [], sql);
    } finally {
      console.warn = original;
    }
    assert.match(warnings.join("\n"), /readonly|read-only|read only|query_only/i, sql);
  }
  assert.equal(count(), 2);
  const db = new DatabaseSync(path, { readOnly: true });
  const table = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='usage'").get();
  db.close();
  assert.ok(table, "the table must still exist");
});

test("the connection has query_only switched on", () => {
  const q = lib.row<Record<string, number>>("PRAGMA query_only");
  assert.equal(Number(Object.values(q ?? {})[0]), 1);
});
