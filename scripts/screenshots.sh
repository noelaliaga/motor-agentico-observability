...
#   make screenshots     # in another: writes docs/screenshots/*.png
#
...
cd "$(dirname "$0")/.."
URL="${URL:-http://127.0.0.1:${MOTOR_PORT:-8797}}"
OUT="docs/screenshots"
PROFILE="$(pwd)/.chrome-screenshots"
...
curl -fsS "$URL/activity" | grep -q "acme-webshop" \
  || { echo "No synthetic project on $URL/activity: refusing to screenshot a non-demo database." >&2; exit 1; }
...
capture() {
  local target="$1" url="$2" profile="$PROFILE-$RANDOM"
  "$CHROME" "${FLAGS[@]}" "--user-data-dir=$profile" "--screenshot=$target" "$url" >/dev/null 2>&1 &
  local pid=$!
  for _ in $(seq 1 "${LIMIT_S:-45}"); do
    if [[ -s "$target" ]] || ! kill -0 "$pid" 2>/dev/null; then break; fi
    sleep 1
  done
  sleep 1
  ...
  [[ -s "$target" ]] && echo "· $target" || echo "✗ no capture for $url" >&2
}

for pair in home: money:money tools:tools connections:connections skills:skills \
            activity:activity inventory:inventory memory:memory review:review \
            hermes:machines/hermes openclaw:machines/openclaw; do
  name="${pair%%:*}"; route="${pair#*:}"
  rm -f "$OUT/$name.png"
  capture "$OUT/$name.png" "$URL/$route"
done
